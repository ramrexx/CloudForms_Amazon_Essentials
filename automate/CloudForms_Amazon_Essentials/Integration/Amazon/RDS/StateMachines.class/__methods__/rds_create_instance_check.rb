=begin
 ...
 Description: This method checks that the AWS RDS instances has been created 
              and then creates a generic object
-------------------------------------------------------------------------------
...
def create_generic_object(hash)
  generic_object_definition = $evm.vmdb(:generic_object_definition).find_by_name('RDS')

  log(:info, "found generic object class: #{generic_object_definition.inspect}")
  log(:info, "found generic object class", true)
  ...
  generic_object_hash = {}
  rds_attributes.each do |attr|
    if attr == :db_instance_identifier
      generic_object_hash[:name] = hash[attr]
    end
    generic_object_hash[attr] = hash[attr]
  end

  log(:info, "generic_object_hash: #{generic_object_hash.inspect}")

  generic_object = generic_object_definition.create_object(generic_object_hash)
  log(:info, "created generic object: #{generic_object.inspect}")
  log(:info, "created generic object", true)

  generic_object.add_to_service(@service)
  log(:info, "added generic object to #{@service.name}", true)
end
...
begin
  ...
  rds_create_instance_check_hash = rds.describe_db_instances({:db_instance_identifier => db_instance_identifier})[:db_instances].first.to_h
  ...
  if rds_create_instance_check_hash[:db_instance_status] == 'available'
    ...
    create_generic_object(rds_create_instance_check_hash)
  else
    set_service_custom_variables(rds_create_instance_check_hash)
    status_message = "DB Instance: #{db_instance_identifier} status: #{rds_create_instance_check_hash[:db_instance_status]}"
    log(:info, status_message, true)
